...
import { ArrowLeft, Download, FileText, Layout } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background text-text pb-20 pt-32 px-4 md:px-8">
      ...
      <div className="max-w-4xl mx-auto mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex bg-surface border border-white/10 p-1 self-stretch md:self-auto">
          <button 
            onClick={() => setViewMode('pdf')}
            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 text-[10px] font-mono uppercase tracking-widest transition-colors ${viewMode === 'pdf' ? 'bg-primary text-black font-bold' : 'text-muted hover:text-white'}`}
          >
            <FileText size={12} /> PDF VIEW
          </button>
          <button 
            onClick={() => setViewMode('web')}
            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 text-[10px] font-mono uppercase tracking-widest transition-colors ${viewMode === 'web' ? 'bg-primary text-black font-bold' : 'text-muted hover:text-white'}`}
          >
            <Layout size={12} /> WEB VIEW
          </button>
        </div>
      </div>
